...
void function () {
  var _, addSchemeToAttr, attrIsOk, cleanArticleTags, cleanBadTags, cleanCodeBlocks, cleanEmTags, cleaner, cleanErrantLinebreaks, cleanParaSpans, cleanUnderlines, divToPara, getReplacementNodes, removeAllAttributes, removeBodyClasses, removeDropCaps, removeNodeKeepChildren, removeNodesRegex, removeScriptsStyles, replaceWithPara;
  _ = require('lodash');
  module.exports.textCleaner = cleaner = function (doc) {
  ...
  module.exports.htmlCleaner = cleaner = function (doc) {
    removeBodyClasses(doc);
    cleanArticleTags(doc);
    cleanCodeBlocks(doc);
    removeDropCaps(doc);
  ...
  removeNodeKeepChildren = function (doc, nodeName) {
    return doc(nodeName).each(function () {
      return doc(this).replaceWith(doc(this).children());
    });
  };
  attrIsOk = function (attr) {
    return attr === 'src' || attr === 'href';
  };
  addSchemeToAttr = function (attr) {
    if (attr.startsWith('//'))
      return 'http:' + attr;
    return attr;
  };
  removeAllAttributes = function (doc) {
    return doc('*').each(function () {
      ...
      return function (accum$) {
        var item;
        for (var cache$ = Object.keys(attrs), i$ = 0, length$ = cache$.length; i$ < length$; ++i$) {
          item = cache$[i$];
          if (!attrIsOk(item))
            continue;
          accum$.push(doc(this)[0].attribs[item] = addSchemeToAttr(attrs[item]));
        }
        return accum$;
      }.call(this, []);
    });
